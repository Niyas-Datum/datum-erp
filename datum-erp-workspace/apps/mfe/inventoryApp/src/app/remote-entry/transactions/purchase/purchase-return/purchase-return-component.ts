// Purchase Return Component - Same as Purchase Invoice but with different pageId/voucherId
// Copy the entire purchase-invoice component and adjust for return functionality
...
@Component({
  selector: 'app-purchase-return-component',
  standalone: true,
...
export class PurchaseReturnComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  override async LeftGridInit(): Promise<void> {
    this.pageheading = 'Purchase Return';
  ...
  // Include all the same methods as PurchaseInvoiceComponent
  // For brevity, referencing purchase-invoice component for full implementation
  // All methods are identical - only difference is the page heading and column headers

  handleInvoiceHeaderForm(event: any): any {
    return this.buildTransactionHeader(event?.headerForm || event, event?.footerForm || {});
  }
  ...
  private extractWarehouseInfo(warehouse: any): any {
    ...
    if (typeof warehouse === 'number') {
      ...
      return matched ? { id: matched.id, value: matched.name || matched.value } : { id: warehouse, value: '' };
    }
    if (typeof warehouse === 'object') {
      return { id: warehouse.id, value: warehouse.value || warehouse.name };
    }
    return {};
  }
  ...
  private extractCustomerInfo(customer: any): any {
    ...
    const match = this.invoiceHeader?.customerData?.find((c) => c.accountName === customer || c.id === customer);
    return match ? { id: match.id, name: match.accountName, code: match.accountCode, description: '' } : {};
  }
  ...
  private extractProjectInfo(project: any): any {
    ...
    const match = this.invoiceHeader?.projectData?.find((p) => p.projectname === project || p.id === project);
    return match ? { id: match.id, name: match.projectcode, code: match.projectname, description: '' } : {};
  }
  ...
  private extractPayType(paytype: any): any {
    ...
    if (typeof paytype === 'object') {
      return { id: paytype.id || null, value: paytype.value || paytype.name || null };
    }
  ...
  private extractIdValue(value: any): any {
    ...
    if (typeof value === 'object') {
      return { id: value.id || null, value: value.value || value.name || null };
    }
  ...
  private extractSalesmanInfo(salesman: any): any {
    ...
    const match = additionalDetails.salesmanData?.find((s: any) => s.name === salesman || s.id === salesman);
    return match ? { id: match.id, name: match.name, code: match.code || '', description: '' } : {};
  }
  ...
  private extractVehicleInfo(vehicleNo: any): any {
    ...
    const additionalDetails = this.additionalDetailsRef;
    if (!additionalDetails) return {};
    const match = additionalDetails.vehicleNoData?.find((v: any) => v.vehicleNo === vehicleNo || v.id === vehicleNo);
    return match ? { id: match.id, name: match.name || '', code: match.vehicleNo || '', description: '' } : {};
  }
  ...
  private extractDeliveryLocationInfo(location: any): any {
    ...
    const additionalDetails = this.additionalDetailsRef;
    if (!additionalDetails) return {};
    const match = additionalDetails.deliveryLocationData?.find((l: any) => l.locationname === location || l.id === location);
    return match ? { id: match.id, name: match.locationname || '', code: match.projectname || '', description: '' } : {};
  }

  hasUnsavedData(): boolean {
    const items = this.commonService.tempItemFillDetails();
    const nonEmptyItems = items.filter((item: any) => item.itemCode && item.itemCode.trim() !== '');
    if (nonEmptyItems && nonEmptyItems.length > 0) return true;
    ...
    if (this.invoiceHeader?.salesForm) {
      ...
      if (!this.initialHeaderValues || Object.keys(this.initialHeaderValues).length === 0) return false;
      const hasHeaderChanges = Object.keys(currentHeader).some(key => {
        ...
        if ((current === null || current === undefined || current === '') && 
            (initial === null || initial === undefined || initial === '')) return false;
    ...
    if (this.invoiceFooter?.salesForm) {
      ...
      if (!this.initialFooterValues || Object.keys(this.initialFooterValues).length === 0) return false;
      ...
      if (hasFooterChanges) return true;
    }
    return false;
  }
  ...
  private loadTransactionDetails(purchaseId: number): void {
    ...
    this.baseService.get(endpoint)
      .pipe(takeUntil(this.destroySubscription), finalize(() => this.isLoading.set(false)))
      .subscribe({
        next: (response: any) => {
          if (response?.data) this.populateTransactionData(response.data);
        },
        error: (error: any) => {
          this.baseService.showCustomDialoguePopup('Failed to load transaction details. Please try again.', 'Load Error', 'ERROR');
        }
      });
  }
  ...
  private confirmExitIfNewlyAddedRows(modeName: string): boolean {
    ...
    return confirm(`You have ${newlyAddedCount} newly added row(s) that will be deleted when exiting ${modeName}. Do you want to continue?`);
  }
  ...
  protected override FormInitialize(): void {
    this.captureInitialValues();
    if (this.invoiceHeader?.salesForm) this.invoiceHeader.salesForm.markAsPristine();
    if (this.invoiceFooter?.salesForm) this.invoiceFooter.salesForm.markAsPristine();
    console.log('FormInitialize completed for PurchaseReturnComponent');
  }
  ...
  private validateTransaction(): any {
    ...
    const items = this.commonService.tempItemFillDetails();
    return this.salesValidator.validateBeforeSave(headerForm, footerForm, additionalForm, items);
  }

  private handleValidationError(result: any): void {
    this.baseService.showCustomDialoguePopup(result.firstError?.message ?? 'Validation failed. Please review the form.', 'WARN');
  }
  ...
  private buildTransactionItems(itemDetails: any[]): any[] {
    return itemDetails.filter((item) => item.itemCode?.trim()).map((item) => this.buildTransactionItem(item));
  }
  ...
  private buildUnitInfo(item: any): any {
    if (item.unit && typeof item.unit === 'object') return item.unit;
    return { unit: item.unit || '', factor: 1, basicunit: item.unit || '' };
  }
  ...
  private calculateTotalGrossAmount(items: any[]): string {
    return items.reduce((sum, item) => sum + this.parseFloatOrZero(item.grossAmt), 0).toFixed(4);
  }
  ...
  private handleSaveSuccess(response: SaveResponse): void {
    if (response?.httpCode === 200) {
      if (response.transactionId && response.transactionId > 0) {
        ...
        setTimeout(() => this.loadTransactionDetails(response.transactionId!), 500);
      }
      this.baseService.showCustomDialoguePopup('Transaction saved successfully!', 'SUCCESS');
      this.isNewMode.set(false);
      this.isEditMode.set(false);
      this.commonService.newlyAddedRows.set([]);
    } else {
      this.baseService.showCustomDialoguePopup(response?.data || 'Save failed. Please try again.', 'WARN');
    }
  }

  private handleSaveError(err: any): void {
    console.error('Save failed', err);
    this.baseService.showCustomDialoguePopup(err?.error?.data || err?.message || 'Save failed. Please check console for details.', 'Save Failed', 'WARN');
  }
  ...
  onNewClick(): void {
    if (this.isNewMode()) {
      if (this.hasUnsavedData()) {
        this.viewDialog('You have unsaved changes. Do you want to discard them and exit new mode?', 'Unsaved Changes', '450px', [
          { click: this.confirmDiscardAndExit.bind(this), buttonModel: { content: 'Yes', isPrimary: true } },
  ...
  onDeleteClick(): void {
    ...
    this.viewDialog('Are you sure you want to delete this transaction? This action cannot be undone.', 'Delete Confirmation', '450px', [
      { click: this.confirmDelete.bind(this), buttonModel: { content: 'Yes', isPrimary: true } },
      { click: this.hideBtnClick.bind(this), buttonModel: { content: 'No' } }
    ]);
  }
  ...
  private performDelete(): void {
    ...
    this.isLoading.set(true);
    this.transactionService.deleteDetails(endpoint)
      .pipe(takeUntil(this.destroySubscription), finalize(() => this.isLoading.set(false)))
      .subscribe({
        next: (response: any) => {
          if (response?.httpCode === 200) {
            this.baseService.showCustomDialoguePopup(response.data || 'Transaction deleted successfully!', 'SUCCESS');
            this.selectedTransactionId.set(null);
            this.isNewMode.set(false);
            this.isEditMode.set(false);
            this.LeftGridInit();
            ...
            this.baseService.showCustomDialoguePopup(response.data || 'Failed to delete transaction', 'ERROR');
          }
        },
        error: (error) => {
          console.error('Error deleting transaction:', error);
          this.baseService.showCustomDialoguePopup('An error occurred while deleting the transaction. Please try again.', 'ERROR');
        },
      });
  }
  ...
  handleSelectedItem(item: any): void {
    if (item && item.type) {
      switch (item.type) {
        case 'additionalDetails': this.handleAdditionalDetailsChange(item.data); break;
        case 'barcode': this.handleBarcodeClick(); break;
        case 'grossAmountEdit': this.handleGrossAmountEditChange(item.data); break;
        case 'approve': this.handleApproveChange(item.data); break;
        default: console.log('Unknown event type:', item.type);
      }
    }
  }
