...
export class DebitNoteComponent extends BaseComponent implements OnInit {
  ...
  debitNoteForm = this.formUtil.thisForm;
  pageId = 252;
  ...
  readonly voucherId = 76; // Debit Note VoucherId
  ...
  override FormInitialize() {
    this.debitNoteForm = new FormGroup({
      ...
      supplierId: new FormControl({ value: 0, disabled: false }), // Hidden field
      supplierName: new FormControl({ value: '', disabled: false }, Validators.required),
      particulars: new FormControl({ value: '', disabled: false }),
    });
    this.formUtil.thisForm = this.debitNoteForm;
  }

  override SaveFormData() {
    try {
      const isUpdate = !!(this.selectedDebitNoteId && this.selectedDebitNoteId > 0);

      if (!this.debitNoteForm || this.debitNoteForm.invalid) {
        this.debitNoteForm?.markAllAsTouched();
        this.formValidationError();
        return;
      }

      const formVals = this.debitNoteForm.getRawValue();
      ...
      if (!this.selectedSupplierId || this.selectedSupplierId <= 0) {
        this.baseService.showCustomDialogue('Please select a supplier.');
        return;
      }
      ...
      const creditTotal = accountDetails.reduce((s: number, x: any) => s + (Number(x.credit) || 0), 0);
      if (creditTotal <= 0) {
      ...
      // Get supplier object
      const selectedSupplier = this.supplierData.find(c => c.id === this.selectedSupplierId);
      const partyPayload = selectedSupplier ? {
      ...
      console.log('Debit Note ID:', this.selectedDebitNoteId);
      console.log('URL:', url);
        ...
        .subscribe({
          next: (response) => {
            ...
            if ((typeof httpCode === 'number' && httpCode >= 400) || isValid === false) {
              ...
              console.error('Save Debit Note returned error payload:', response);
              this.baseService.showCustomDialogue(`Save failed:\n${details}`);
              return;
            }

            const msg = (response as any)?.data?.msg || 'Debit note saved successfully';
            this.baseService.showCustomDialogue(msg);
            // After save, switch to view mode to disable Save button
            this.SetPageType(3);
            this.updateGridEditSettings();
            this.debitNoteForm.disable({ emitEvent: false });
            this.LeftGridInit();
          },
          error: (error) => {
            const details = this.stringifyError(error);
            console.error('Save Debit Note failed:', error);
            this.baseService.showCustomDialogue(`Save failed:\n${details}`);
          },
        });
    } catch (err: any) {
      const details = this.stringifyError(err);
      console.error('Save Debit Note exception:', err);
      this.baseService.showCustomDialogue(`Save failed:\n${details}`);
    }
  }
  ...
  override getDataById(data: PVoucherModel) {
    ...
    this.selectedDebitNoteId = data.ID;

    this.SetPageType(3);
    this.updateGridEditSettings();
    this.debitNoteForm.disable();
    this.fetchDebitNoteById();
  }

  override onEditClick() {
    const selectedId = this.selectedDebitNoteId || (this as any).leftgridSelectedData?.ID;
    if (!selectedId || Number(selectedId) <= 0) {
      this.baseService.showCustomDialogue('Please select a debit note from the list to edit.');
      return;
    }
    ...
    this.selectedDebitNoteId = Number(selectedId);
    this.SetPageType(2);
    this.updateGridEditSettings();
    this.debitNoteForm.enable();
    this.debitNoteForm.get('voucherName')?.disable({ emitEvent: false });
    this.debitNoteForm.get('voucherNo')?.disable({ emitEvent: false });

    this.voucherService.fetchAccountMaster();

    if (!this.currentDebitNote || this.currentDebitNote.id !== this.selectedDebitNoteId) {
      this.fetchDebitNoteById();
    }
  }

  override formValidationError() {
    const c = this.debitNoteForm?.controls || {} as any;
    const errors: string[] = [];
    if (c['voucherNo']?.invalid) errors.push('Voucher No is required.');
    if (c['voucherDate']?.invalid) errors.push('Voucher Date is required.');
    if (c['supplierName']?.invalid) errors.push('Supplier is required.');
    if (errors.length === 0) {
  ...
  private fetchDebitNoteById(): void {
    this.httpService
      .fetch<any>(EndpointConstant.FILLPURCHASEBYID + this.selectedDebitNoteId)
      .pipe(takeUntilDestroyed(this.serviceBase.destroyRef))
      .subscribe({
        next: (response) => {
          ...
          this.currentDebitNote = transactionData ?? null;
          debugger;

          let formVoucherDate = null;
          if (this.currentDebitNote?.date) {
            formVoucherDate = this.datePipe.transform(
              new Date(this.currentDebitNote.date),
              'dd/MM/yyyy'
            );
          }

          // Extract supplier and particulars
          const supplierId = this.currentDebitNote.accountID || 0;
          ...
          this.debitNoteForm.patchValue({
            ...
            voucherNo: this.currentDebitNote?.transactionNo,
            voucherDate: formVoucherDate,
            narration: this.currentDebitNote?.commonNarration,
            referenceNo: this.currentDebitNote?.referenceNo,
          ...
          const accountDetails = transactionEntries
            ...
            .map((entry: any) => {
              ...
              const accountRow: any = {
                ...
                originalCredit: entry.credit || 0,
                originalAccountId: accountId
              };

              // Restore PO allocations for this supplier
              const poAllocationsForAccount = billAndRefData
                .filter((bill: any) => Number(bill.AccountID ?? bill.accountID) === Number(supplierId))
                .map((bill: any) => {
  ...
  private enterNewMode(): void {
    console.log('Entering New mode for debit note');
    ...
    console.log('Form reset for new debit note');
  }
  ...
  private viewTopVoucherFromLeftGrid(): void {
    ...
    if (!firstVoucher) {
      this.baseService.showCustomDialogue('No debit notes available to view.');
      return;
    }
  ...
  override DeleteData(data: PVoucherModel) {
    console.log('Delete requested for debit note:', data);

    if (!data || !data.ID) {
      this.baseService.showCustomDialogue('Please select a valid debit note to delete.');
      return;
    }
    ...
    const confirmDelete = confirm(
      `Are you sure you want to delete Debit Note "${voucherNo}"?\n\n` +
  ...
  // Supplier dropdown selection handler
  onSupplierSelect(args: any): void {
    const selectedSupplier = args.itemData || args.item;

    if (!selectedSupplier) return;

    const previousSupplierId = this.selectedSupplierId;
    this.selectedSupplierId = selectedSupplier.id || 0;

    // If supplier changed and there are existing allocations, warn and clear
    if (previousSupplierId && previousSupplierId !== this.selectedSupplierId) {
      this.clearAllPOAllocations();
      console.warn('Supplier changed - all PO allocations cleared');
    }

    // Store supplier ID in form
    this.debitNoteForm.patchValue({ supplierId: this.selectedSupplierId });

    console.log('Supplier selected:', selectedSupplier.name, 'ID:', this.selectedSupplierId);
  }

  // Handle filtering in Supplier dropdown - search by both code and name
  onSupplierFiltering(args: any): void {
    if (!args?.text) return;

    const query = args.text.toLowerCase();
    const filtered = this.supplierData.filter((supplier: any) =>
      supplier?.code?.toLowerCase().includes(query) ||
      supplier?.name?.toLowerCase().includes(query)
    );

    args.updateData(filtered.length ? filtered : this.supplierData);
  }

  // Clear all PO allocations when supplier changes
  private clearAllPOAllocations(): void {
    ...
    this.debitNoteForm.patchValue({ referenceNo: '' });
    console.log('All PO allocations cleared due to supplier change');
  }
  ...
  private getVoucherDateForEditCheck(): Date | null {
    const rawDate = this.currentDebitNote?.date || this.debitNoteForm.get('voucherDate')?.value;
  ...
  // Fetch supplier data
  private fetchSupplierData(): void {
  ...
  async onAccountDetailsCellSave(args: any): Promise<void> {
    ...
    // CRITICAL: Check if CREDIT column was edited (Debit Note uses Credit, not Debit)
    if (columnName === 'credit') {
      const creditValue = value || 0;

      // Validate that supplier is selected first
      if (!this.selectedSupplierId || this.selectedSupplierId <= 0) {
        this.baseService.showCustomDialogue('Please select a supplier first before entering credit amount.');
        return;
      }
      ...
      if (isEditMode) {
        ...
        // In new mode, just fetch unpaid POs for the selected SUPPLIER
        await this.fetchUnpaidPOsForSupplier(this.selectedSupplierId);
      }

      // Check if supplier has unpaid POs
      if (this.voucherService.unpaidPOsData().length > 0) {
        setTimeout(() => {
          this.openPOAllocationPopup(creditValue, rowData);
        }, 100);
      } else {
        console.log(`ℹ️ No unpaid POs for supplier ID ${this.selectedSupplierId}. Manual credit entry: ${creditValue}`);
      }
    }
  }

  // Fetch unpaid POs for supplier (CRITICAL: Uses supplierId, not accountId)
  private async fetchUnpaidPOsForSupplier(supplierId: number): Promise<void> {
    if (!supplierId || supplierId <= 0) {
      console.warn('Invalid supplier ID for fetching unpaid POs');
      return;
    }

    // Fetch unpaid invoices using voucherId=17 (Invoice ID) and drcr=C (same as Payment Voucher)
    const url = `${EndpointConstant.FILLADVANCE}${supplierId}&voucherId=17&drcr=C`;
    ...
    try {
      ...
      console.log(`✅ Fetched ${unpaidPOs.length} unpaid POs for supplier ID ${supplierId}`);
    } catch (err) {
      console.error('Error fetching unpaid POs for supplier:', err);
      this.voucherService.unpaidPOsData.set([]);
    }
  }

  // Prepare unpaid POs for edit mode - merge with existing allocations (similar to Payment Voucher)
  private async prepareUnpaidPOsForEdit(rowData: any): Promise<void> {
    ...
    // Fetch unpaid POs for this supplier
    if (supplierId) {
      const url = `${EndpointConstant.FILLADVANCE}${supplierId}&voucherId=17&drcr=C`;
    ...
    // Get allocated POs from all rows (for debit note, all rows share same supplier)
    const accountRows = this.voucherCommonService.accountDetailsData() || [];
    ...
    console.log(`✅ Prepared ${merged.length} POs for edit mode (supplier ID: ${supplierId})`);
  }

  // Open PO Allocation popup
  openPOAllocationPopup(creditAmount: number, rowData: any): void {
    this.currentRowData = rowData;
    this.poAllocationPopup.open(creditAmount);
  }
  ...
  onAllocationComplete(result: POAllocationResult): void {
    ...
    this.debitNoteForm.get('referenceNo')?.setValue(result.invoiceNosString);
  ...
  private updateReferenceFieldOnAccountChange(clearedRow: any): void {
    ...
    this.debitNoteForm.get('referenceNo')?.setValue(newReferenceNo);
  ...
  private deleteCreditNote(voucherId: number, voucherNo: string): void {
    ...
    console.log('Deleting debit note:', { voucherId, voucherNo, pageId: this.pageId, url: deleteUrl });
  ...
  private handleDeleteSuccess(response: any, voucherNo: string): void {
    ...
    if ((typeof httpCode === 'number' && httpCode >= 400) || isValid === false) {
      const details = typeof dataMsg === 'string' ? dataMsg : this.stringifyError(response);
      console.error('Delete debit note returned error:', response);
      this.baseService.showCustomDialogue(`Delete failed:\n${details}`);
      return;
    }

    const successMsg = dataMsg || `Debit Note "${voucherNo}" deleted successfully.`;
  ...
  private handleDeleteError(error: any, voucherNo: string): void {
    const details = this.stringifyError(error);
    console.error('Delete debit note failed:', error);

    this.baseService.showCustomDialogue(
      `Failed to delete Debit Note "${voucherNo}":\n${details}`
    );
  }
