...
import { BaseService, validName } from "@org/services";
...
export class LedgerComponent extends BaseComponent implements OnInit {
  ...
  override FormInitialize() {
    this.ledgerForm = new FormGroup({
      ...
      AccountCode: new FormControl({ value: '', disabled: this.isInputDisabled }, Validators.required),
      AccountName: new FormControl({ value: '', disabled: false }, [Validators.required, validName]),
  ...
  override SaveFormData() {
    ...
    if (this.ledgerForm.get('AccountCode')?.value === null) {
      this.toast.error("Please enter Account Code");
      return
    }
    if (this.ledgerForm.get('AccountName')?.value === null) {
    ...
    const payload: any = {
      ...
      active: this.ledgerForm.get('active')?.value ? this.ledgerForm.get('active')?.value : false,
      preventExtraPay: this.ledgerForm.get('PreventExtraPay')?.value,
      maintainIteamWise: this.ledgerForm.get('maintainitemwise')?.value ? this.ledgerForm.get('maintainitemwise')?.value : false,
      ...
      isGroup: this.ledgerForm.get('isGroup')?.value ? this.ledgerForm.get('isGroup')?.value : false
    }
    const savepayload = {
      group: {
        id: this.selectedGroupId,
